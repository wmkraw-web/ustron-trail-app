    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        <div className="md:col-span-2 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-3xl p-6 md:p-8 text-white shadow-xl flex justify-between items-center relative overflow-hidden">
          ...
          <div className="relative z-10"><h2 className="text-2xl md:text-4xl font-bold mb-1">Dzień dobry!</h2><p className="text-blue-100 mb-6 text-sm">Idealna pogoda na góry.</p></div>
        </div>
      </div>
      <div>
...
function ChatAssistantView() {
  ...
  const handleSend = async () => {
    if (!msg.trim()) return;
    setChat([...chat, { role: 'user', text: msg }, { role: 'ai', text: 'Funkcja chatu z AI działa na backendzie Vercel (/api/chat).' }]);
    setMsg("");
  };
  return (
...
function JournalView({ savedTrips, activeTrip, setActiveTrip, onAddMedia }) {
  ...
  const handleFileUpload = (e) => {
    ...
    files.forEach(file => { const url = URL.createObjectURL(file); onAddMedia(activeTrip.id, url); });
  };
  if (activeTrip) {
      return (
          <div className="p-8 max-w-5xl mx-auto space-y-6">
              <button onClick={() => setActiveTrip(null)} className="flex items-center gap-2 text-emerald-600 font-bold"><ArrowLeft size={20} /> Powrót</button>
              <div className="bg-white rounded-3xl p-10 border shadow-sm">
                  <h2 className="font-black text-3xl mb-6">{activeTrip.name}</h2>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {activeTrip.media.map((url, idx) => (<img key={idx} src={url} className="aspect-square object-cover rounded-2xl" />))}
                      <div onClick={() => fileInputRef.current?.click()} className="aspect-square bg-emerald-50 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center cursor-pointer"><Upload size={32} /><input type="file" ref={fileInputRef} className="hidden" multiple onChange={handleFileUpload} /></div>
                  </div>
              </div>
          </div>
      );
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h2 className="font-black text-2xl mb-6">Moje Wyprawy</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {savedTrips.map(trip => (
          <div key={trip.id} onClick={() => setActiveTrip(trip)} className="bg-white rounded-2xl p-5 border cursor-pointer hover:border-emerald-500">
            <h3 className="font-bold text-lg">{trip.name}</h3><p className="text-sm text-slate-500">{trip.date}</p>
          </div>
        ))}
      </div>
    </div>
...
function SidebarItem({ icon, label, isActive, onClick }) { return <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive ? 'bg-emerald-800 text-white font-bold' : 'text-emerald-200 hover:bg-emerald-800/50'}`}>{icon} <span>{label}</span></button>; }
function NavItem({ icon, label, isActive, onClick }) { return <button onClick={onClick} className={`flex flex-col items-center w-[72px] gap-1 ${isActive ? 'text-emerald-600' : 'text-slate-400'}`}><div className={`p-1.5 rounded-full ${isActive ? 'bg-emerald-50 scale-110' : ''}`}>{icon}</div><span className="text-[10px] font-bold">{label}</span></button>; }
function QuickActionButton({ icon, label, color, iconColor, onClick }) { return <button onClick={onClick} className={`${color} p-4 rounded-2xl flex flex-col items-center justify-center gap-2 shadow-sm`}><div className={`${iconColor} text-white p-3 rounded-full`}>{icon}</div><span className="font-bold text-xs">{label}</span></button>; }
